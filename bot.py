...
def get_online_devices(user_id):
    """Fetch online devices with automatic SIM fallback"""
    ...
    for dev_id, info in data.items():
        if info.get("status") == True:
            ...
            # 🔥 FIX: If no SIMs, create a default one
            if not sims or len(sims) == 0:
                logger.warning(f"No SIMs found for device {dev_id}, creating default SIM")
                sims = [{
                    "simSlotIndex": 0,
                    "phoneNumber": "Unknown",
                    "isDefault": True
...
def send_sms_command(user_id, device_id, to_number, message, from_number):
    """Send SMS command with default fallback"""
    # 🔥 FIX: If from_number is "Unknown" or empty, use default
    if not from_number or from_number == "Unknown":
        from_number = "Default"
        logger.info(f"Using default SIM for SMS send")
    ...
    logger.info(f"📤 SMS command sent: device {device_id} -> {to_number} (from: {from_number})")
...
async def device_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 FIX: Always ensure we have at least one SIM option
    sims = device_data.get("sims", [])
    if not sims:
        # Create default SIM if none exist
        sims = [{"simSlotIndex": 0, "phoneNumber": "Default SIM"}]
        logger.info(f"Created default SIM for device {device_id}")
    
    keyboard = []
    for sim in sims:
        slot = sim.get("simSlotIndex", 0)
